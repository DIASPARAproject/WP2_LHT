...
dfj <- laa.samples %>%
  ...
  ungroup() %>%
  left_join(data.j %>%
              select(length_mm_j,spat.unit,g.year) %>%
...
dfj %>%
  ggplot() +
  geom_abline(slope = 1) +
  geom_point(aes(x = obs.l, y = pred.l, color = factor(g.year)),shape = 4,alpha = 0.5) +
  theme_minimal() +
  facet_wrap(~spat.unit) +
  labs(caption = "Juvenile length at age" )
ggsave(paste0(home, "/report/salaa_poj.png"), width = 17, height = 14, units = "cm")

# jrmse <- laa.samples %>% # rmse = sd of residuals
#   spread_draws(l.mu.j[i], sep = ",") %>%
#   rename(pred.l = l.mu.j,
#          ind.id = i) %>%
#   left_join(data.j %>%
...
#               mutate(ind.id = row_number()), by = "ind.id") %>%
#   group_by(.draw) %>%
#   summarise(rmse = sqrt(mean((obs.l - pred.l)^2)))
...
laa.samples %>%
  ...
  left_join(var.lats) %>%
  mutate(spat.unit = reorder(spat.unit, m.lat)) %>%
  ggplot() +
  geom_boxplot(aes(x = .value, y = spat.unit),
               outliers = FALSE
               ) +
  theme_minimal() +
  labs(y = "", x = expression(l[b]))
...
laa.samples %>%
  ...
  mutate(spat.unit = reorder(spat.unit, m.lat)) %>%
  ggplot() +
  geom_boxplot(aes(x = exp(.value), y = spat.unit),
               outliers = FALSE) +
  theme_minimal() +
  labs(y = "", x = expression(r))
...
laa.samples %>%
  ...
  left_join(var.lats) %>%
  mutate(sex = ifelse(sex == 1, "f","m"),
         spat.unit = reorder(spat.unit, m.lat)) %>%
  ggplot() +
  ...
  labs(y = "", x = "k")
ggsave(paste0(home, "/report/salaa_k.png"), width = 17, height = 14, units = "cm")
...
#dfp <- laa.samples[[1]][1:1000, ] %>% # sample from one chain to reduce object size
dfp <- laa.samples[[1]] %>% # sample from one chain to reduce object size
...
dfp %>%
  ...
  filter(sex == "f", 
         coh %in% cohs) %>% # coh = 1 is 1998
  left_join(data.biph %>% distinct(su, spat.unit)) %>%
  ggplot() +
  geom_line(aes(x = gy, y = l.mu, color = factor(smo.age)), linewidth = 0.1) +
  geom_pointinterval(
    aes(x = gy, y = l.mu, ymin = .lower, ymax = .upper, color = factor(smo.age)),
    point_size = 0.5,
    linewidth = 1
  ) +
  facet_wrap(~spat.unit) +#, scales = "free") +
  theme_minimal() +
  labs(y = expression(mu[L]), x = expression(age[g]), color = "smolt age")
ggsave(paste0(home, "/report/salaa_cfsa.png"), width = 17, height = 14, units = "cm")
...
dfp %>%
  ...
  theme_minimal() +
  labs(y = expression(mu[L]), x = expression(age[g]), color = "smolt age")
ggsave(paste0(home, "/report/salaa_cmsa.png"), width = 17, height = 14, units = "cm")
...
dfp %>%
  filter(gy <= masu[su,2])  %>%
  filter(smo.age == 2) %>%
  semi_join(smcsu, by = c("su", "smo.age", "sex")) %>%
  mutate(sex = if_else(sex == 1, "f","m")) %>%
  filter(sex == "m",
         coh %in% cohs
         ) %>% 
  ...
  mutate(coh = coh + 1998) %>%
  ggplot() +
  geom_line(aes(x = gy, y = l.mu, color = factor(coh)), linewidth = .3) +
  facet_wrap(~spat.unit) +
  theme_minimal() +
  labs(y = expression(mu[L]), x = expression(age[g]), color = "cohort")
ggsave(paste0(home, "/report/salaa_cmc.png"), width = 17, height = 14, units = "cm")
...
dfp %>%
  ...
  theme_minimal() +
  labs(y = expression(mu[L]), x = expression(age[g]), color = "cohort")
ggsave(paste0(home, "/report/salaa_cfc.png"), width = 17, height = 14, units = "cm")
...
as.data.frame(corr) %>%
  ...
  labs(x = "", y = "")
ggsave(paste0(home, "/report/salaa_cor.png"), width = 17, height = 14, units = "cm")
