...
```{r}
# Load samples 
schnu.samples <- readRDS(paste0(home,"/models_eel/samples/schnu12_samples_2026-02-11.RData"))$samples #v.11
...
cor_draws <- schnu.samples %>%
  spread_draws(bpar[mb, p], sep = ",") %>%
  pivot_wider(names_from = p, values_from = bpar, names_prefix = "par") %>%
  group_by(.draw) %>%
  summarize(corr_P_k = cor(par2, par4),
            corr_P_L1 = cor(par2, par1),
            corr_P_L2 = cor(par2, par3),
            corr_k_L1 = cor(par4, par1),
            corr_k_L2 = cor(par4, par3),
            corr_L1_L2 = cor(par1, par3)) %>%
  pivot_longer(cols = 2:7, names_to = "pars", values_to = "corr") 

cor_draws %>%
  ggplot(aes(x = corr)) +
  geom_histogram(fill = "grey") +
  geom_vline(xintercept = mean(cor_draws$corr_P_k), color = "darkgrey", linetype = "dashed") +
  facet_wrap(~pars)
  # labs(#x = "Correlation between L2 and K across basins",
  #      #y = "Posterior frequency",
  #      title = "Spatial correlation P and K")
```
